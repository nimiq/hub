...
import patchMerkleTree from '../src/lib/MerkleTreePatch';
...
class Demo {
    public static run() {
        ...
        document.querySelector('button#sign-multisig-transaction')!.addEventListener('click', async () => {
            const signer = (document.querySelector('#multisig-signer') as HTMLInputElement).value;
            const publicKey = (document.querySelector('#multisig-publickey') as HTMLInputElement).value;
            if (!signer || publicKey.length !== 64) {
                alert('Connect an Account first');
