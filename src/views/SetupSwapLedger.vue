...
export default class SetupSwapLedger extends Mixins(SetupSwap, SetupSwapSuccess) {
    ...
    protected created() {
        ...
        Promise.all([
            ...
            // preload bitcoinjs-lib/src/transaction and electrum used in BTC funding and redeeming
            ...(fund.type === SwapAsset.BTC || redeem.type === SwapAsset.BTC
                ? [import('bitcoinjs-lib/src/transaction'), getElectrumClient()]
                : []),
            // preload bitcoinjs-lib/src/networks, bitcoinjs-lib/src/payments and Buffer used in BTC funding
            ...(fund.type === SwapAsset.BTC
                ? [import('bitcoinjs-lib/src/networks'), import('bitcoinjs-lib/src/payments'), import('buffer')]
                : []),
            // preload bitcoinjs-lib/src/script used in BTC redeeming
            redeem.type === SwapAsset.BTC ? import('bitcoinjs-lib/src/script') : null,
    ...
    protected async _collectSwapSetupInfo(): Promise<SwapSetupInfo | null> {
        ...
        if (this._nimiqSwapProxyPromise) {
            ...
            if (swapSetupInfo.fund.type === SwapAsset.NIM) {
                ...
                swapSetupInfo.fund.senderType = Nimiq.AccountType.Basic;
    ...
    protected async _signSwapTransactions(htlcInfo: SwapHtlcInfo)
        ...
        if (this.request.fund.type === SwapAsset.NIM
            ...
            nimiqSwapTransactionInfo = {
                value: BigInt(swapSetupInfo.fund.value),
                fee: BigInt(swapSetupInfo.fund.fee),
        ...
        let signedBitcoinTransaction: BitcoinJsTransaction | undefined;
        ...
        try {
            ...
            if (swapSetupInfo.fund.type === SwapAsset.NIM && nimiqProxyTransactionInfo && this.nimiqLedgerAddressInfo) {
                ...
                signedNimiqProxyTransaction = this.nimiqNetwork.getUnrelayedTransactions({
                    ...nimiqProxyTransactionInfo,
            ...
            if (bitcoinTransactionInfo && preparedBitcoinTransactionInfoPromise) {
                ...
                signedBitcoinTransaction = BitcoinJsTransaction.fromHex(
        ...
        if (swapSetupInfo.redeem.type === SwapAsset.BTC
            ...
            const { fromASM: scriptFromAsm, toStack: scriptToStack } = await import('bitcoinjs-lib/src/script');
    ...
    private _toCoins(amount: number, currency: SwapAsset) {
        switch (currency) {
            case SwapAsset.NIM: return amount / 1e5;
