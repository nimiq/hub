<template>
    <div v-if="showAccountSelector" class="container">
        <SmallPage>
            ...
            <PageBody>
                <div class="account-selector">
                    <button v-for="wallet of wallets" :key="wallet.id"
                        ...
                        :disabled="wallet.type === WalletType.LEDGER"
                    >
                        <div v-if="wallet.type === WalletType.BIP39" class="icon"
                            :class="backgroundClass(wallet.accounts.values().next().value.address.toUserFriendlyAddress())"
                        ><LoginFileIcon /></div>
                        <Identicon v-else-if="wallet.type === WalletType.LEGACY" class="icon"
                            :address="wallet.accounts.values().next().value.address.toUserFriendlyAddress()"/>
                        <div v-else-if="wallet.type === WalletType.LEDGER" class="icon">
                            <LedgerIcon />
                        </div>
            ...
            <PageFooter>
                <button class="nq-button-s" @click="goToOnboarding">Log into other Accounts</button>
            </PageFooter>
        </SmallPage>
...
export default class ConnectAccount extends Vue {
    ...
    private async setWallet(wallet: WalletInfo, userSelected = true) {
        if (wallet.type === WalletType.LEDGER) return;
