...
export class CookieJar {
    ...
    public static encodeString(string: string, buffer: Nimiq.SerialBuffer) {
        const bytes = Utf8Tools.stringToUtf8ByteArray(string);
        buffer.writeVarUint(bytes.length);
        buffer.write(bytes);
    }

    public static decodeString(buffer: Nimiq.SerialBuffer): string {
        const size = buffer.readVarUint();
        return Utf8Tools.utf8ByteArrayToString(buffer.read(size));
    }

    public static decodeElements(buffer: Nimiq.SerialBuffer) {
    ...
    public static encodeNumber(number: number | undefined, buffer: Nimiq.SerialBuffer) {
    ...
    public static decodeNumber(buffer: Nimiq.SerialBuffer): number | undefined {
        return (buffer.readUint8() == 1) ? buffer.readVarUint() : undefined;
    }

    public static encodeAddress(address: Uint8Array, buffer: Nimiq.SerialBuffer) {
        buffer.write(address);
    }

    public static decodeAddress(buffer: Nimiq.SerialBuffer): Uint8Array {
        return buffer.read(20);
    }

    public static encodePath(path: string, buffer: Nimiq.SerialBuffer) {
    ...
    public static decodePath(buffer: Nimiq.SerialBuffer): string {
        return buffer.readVarLengthString();
    }

    public static encodeId(hex: string, buffer: Nimiq.SerialBuffer) {
        // example "1ee3d926a49c"
        if (hex.length != 12) throw new Error(`not a valid ID ${hex}`);
        buffer.writeVarUint(parseInt(hex, 16));
    }
    public static decodeId(buffer: Nimiq.SerialBuffer): string {
        return buffer.readVarUint().toString(16);
    }

    public static encodeAccount(account: AccountInfoEntry, buffer: Nimiq.SerialBuffer) {
    ...
    public static decodeAccount(buffer: Nimiq.SerialBuffer): AccountInfoEntry {
        return {
    ...
    public static encodeAccounts(accountsMap: Map</*address*/ string, AccountInfoEntry>, buffer: Nimiq.SerialBuffer) {
        const accounts = Array.from(accountsMap.values());
        buffer.writeUint8(accounts.length);
        accounts.forEach((account) => this.encodeAccount(account, buffer));
    }

    public static decodeAccounts(buffer: Nimiq.SerialBuffer): Map</*address*/ string, AccountInfoEntry> {
    ...
    public static encodeContractType(type: ContractType, buffer: Nimiq.SerialBuffer) {
        buffer.writeUint8(type.valueOf());
    }

    public static decodeContractType(buffer: Nimiq.SerialBuffer): ContractType {
        return buffer.readUint8() as ContractType;
    }

    public static encodeContract(contract: ContractInfo, buffer: Nimiq.SerialBuffer) {
    ...
    public static decodeContract(buffer: Nimiq.SerialBuffer): ContractInfo {
        return {
            address: this.decodeAddress(buffer),
    ...
    public static encodeContracts(contracts: ContractInfo[], buffer: Nimiq.SerialBuffer) {
        buffer.writeUint8(contracts.length);
        contracts.forEach((contract) => this.encodeContract(contract, buffer));
    }

    public static decodeContracts(buffer: Nimiq.SerialBuffer): ContractInfo[] {
        return this.decodeElements(buffer).map((x) => this.decodeContract(buffer));
    }

    public static encodeType(type: WalletType, buffer: Nimiq.SerialBuffer) {
        buffer.writeUint8(type.valueOf());
    }

    public static decodeType(buffer: Nimiq.SerialBuffer): WalletType {
        return buffer.readUint8() as WalletType;
    }

    public static encodeWallet(wallet: WalletInfoEntry, buffer: Nimiq.SerialBuffer) {
    ...
    public static decodeWallet(buffer: Nimiq.SerialBuffer): WalletInfoEntry {
        return {
    ...
    public static encode(wallets: WalletInfoEntry[]) {
        const buffer = this.encodeWallets(wallets);
        return Nimiq.BufferUtils.toBase64(buffer);
    }

    public static decode(data: string): WalletInfoEntry[] {
    ...
    public static requiredBytes(wallets: WalletInfoEntry[]) {
