// Manually reference type dependency, because our old typescript version doesn't seem to be finding it automatically.
...
export async function getHttpClient() {
    const { HttpClient, HttpWrapper } = await import('@opengsn/common');
    return new HttpClient(new HttpWrapper(), console);
}
...
export function getRelayHub({ ethers, provider }: PolygonClient) {
    return relayHubContract || (relayHubContract = new ethers.Contract(
        Config.polygon.openGsnRelayHubContract,
